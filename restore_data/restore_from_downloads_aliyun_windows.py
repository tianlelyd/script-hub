...
def read_db_foreign_key_enable():
    ...
    cmd = "mysql -h" + db_host + " -P" + db_port + " -u" + db_user + " -p" + db_pass + " -e \"SELECT @@foreign_key_checks\""
    ...
    try:
        ...
        if len(lines) >= 2:
            foreign_check_enable = lines[1].strip()
        ...
        print(f"[WARN] try to get foreign key config failed: {str(e)}")
        print("[WARN] won't change foreign key config for MySQL")
        enable_foreign_key_check = False
